...
int main(){
  ...
  do{
    printf("\n> %d. particle index (0 for end, 999 for list): ", n_particles + 1);
    ...
    if(cur_particle_no > 0){
      if(cur_particle_no != 999){
    	  ...
    	  for (i = 0; i < PARTICLE_DATA_N; i++){
    		  printf("\nparticle index: %2d --> %s", AT_Particle_Data.particle_no[i],
												   AT_Particle_Data.particle_name[i]);
    	  }
      }
    }
  ...
  long index = RDD_model; // not fully correct, works only if RDD_index = RDD_no!
  ...
  if(AT_GR_Data.n_parameters[index]>0){
	...
	for(i = 0; i < AT_GR_Data.n_parameters[index];i++){
	  printf("\n %s [0 for default: %g]: ", AT_GR_Data.parameter_name[index][i],
											AT_GR_Data.parameter_default[index][i]);
	  scanf("%g", &float_dummy);
