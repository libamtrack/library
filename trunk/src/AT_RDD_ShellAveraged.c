...
double  AT_inverse_RDD_KatzSite_m( const double D_Gy,
  ...
  const double D_crit_Gy = AT_RDD_KatzPoint_Gy(a0_m, r_min_m, r_max_m, er_model, alpha, Katz_point_coeff_Gy);
  if( D_Gy > d_max_Gy )
    return 0.0;
  if( D_Gy <= D_crit_Gy )
    return AT_inverse_RDD_KatzPoint_m(D_Gy, r_min_m, r_max_m, er_model, alpha, Katz_point_coeff_Gy);
  else
    return a0_m;
}
