...
# plotting
pdf("Katz_RBE.pdf", width=15, height=15, pointsize=24)

simulation.results <- read.table("Katz_results.dat")
...
for( z_en in Z_init.energy){
  ...
  LET.vector.Katz <- unique(simulation.results$LET[cond.z_en])
...
#  if ((particle.no > 0) & (Z == 6) & (Init.Energy == 290)) {
  if (particle.no > 0) {
   ...
   legend( "topright", legend = c("exp. data","model - Katz"), pch=c(1,2), col=c("green","red"), lw = 4)

   RBE.vector.Katz <- numeric()
   LET.vector.Katz <- sort( LET.vector.Katz ) 

   for( LET in LET.vector.Katz ){
      ###################### data preparation ###############################
     cat("******* LET = ",LET,"**********\n")
     E.MeV.u <- AT.E.MeV.u( LET , particle.no, material.no )[1] # energy from LET
     cond.LET.Katz  <- ((simulation.results$Z == Z) & (simulation.results$Init.Energy == Init.Energy) & (simulation.results$LET == LET))

     # read dose values from model simulation
     D.Gy <- simulation.results$D.Gy[cond.LET.Katz]
     
     # read dose values from model simulation (gamma radiation)
     Survival.Katz.gamma <- simulation.results$Survival.gamma[cond.LET.Katz]
     ...
     Survival.Katz.ion <- simulation.results$Survival.ion[cond.LET.Katz]
   ...
   ###################### plotting ###############################
   points( x = LET.vector.Katz/10., y = RBE.vector.Katz, col = "red", pch = 2)
...
# plotting
pdf("Katz_SurvivalCurves.pdf", width=15, height=15, pointsize=24)
...
#LET.vector <- unique(simulation.results$LET[cond.z_en])
  ...
  if (particle.no > 0) {
   ...
   for( LET in LET.vector.exp ){
     ...
     Survival.Katz.gamma.LQ <- 100.*exp( - Katz.gamma.params[1] * LQ.D.Gy - Katz.gamma.params[2] * LQ.D.Gy * LQ.D.Gy)
     ...
     Survival.Katz.ion.LQ <- 100.*exp( - Katz.ion.params[1] * LQ.D.Gy - Katz.ion.params[2] * LQ.D.Gy * LQ.D.Gy)
     
     # dose reproducibility factor
     Dose.Factor.Katz <- simulation.results$Dose.Factor[cond.LET.Katz]
     ...
     # plot gamma Katz simulated data and LQ fit
     points( x = D.Gy, y = Survival.Katz.gamma/100., pch = 3, col = "green")
     lines( x = LQ.D.Gy , y = Survival.Katz.gamma.LQ/100. , col = "green")
     
     # plot ion Katz simulated data and LQ fit
     points( x = D.Gy, y = Survival.Katz.ion/100., pch = 4, col = "red")     
     lines( x = LQ.D.Gy , y = Survival.Katz.ion.LQ/100. , col = "red")
