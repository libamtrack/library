 ...
 ii 			<- df1$ER.model == i
	for( j in RDD.model) {
 	jj 			<- ((df1$RDD.model == j) & (df1$ER.model == i)) 
			df1$D.Gy[jj] <- AT.RDD.D.Gy(r.m = df1$r.m[jj], E.MeV.u, particle.no, material.no, ER.model = i, ER.parameters = ER.parameters, RDD.model = j, RDD.parameters=RDD.parameters[[j]] ) 	
		}
}
...
p1a <- xyplot( D.Gy ~ r.m | RDD.model.name, groups = ER.model.name, ref = TRUE, data=df1, pch = ".", lty = 1, type = "l", xlab = "Distance [m]", ylab = "Dose [Gy]", auto.key = list(title = "Protons in liquid water, RDD",points = FALSE, lines = TRUE), scales = list(log = 10))
...
#		df2$r.m[jj] <- AT.RDD.r.m(D.Gy = df2$D.Gy[jj], E.MeV.u, particle.no, material.no, ER.model = i, ER.parameters = ER.parameters, RDD.model = j, RDD.parameters=RDD.parameters[[j]] ) 	
#		}
#}
