/**
 * @brief AT_momentum_from_E wrapper
 */

/*
 *    AT_momentum_from_E.c
 *    ===================
 *
...
int main(int argc, char *argv[]) {
	...
	double * momentum_MeV_c = (double*)calloc(n_points, sizeof(double));
	...
	int status = AT_momentum_MeV_c_u_from_E_MeV_u( n_points,
			E_MeV_u,
			momentum_MeV_c);


	if( status != EXIT_SUCCESS){
		fprintf(stderr, "Exit code from AT_momentum_from_E = %d\n", status);
		fclose(f);
		free(momentum_MeV_c);
		free(E_MeV_u);
		return EXIT_FAILURE;
