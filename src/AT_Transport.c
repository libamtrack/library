...
void  AT_Dose_Gy(  const float*  density_g_cm3,
    const float*  fluence_cm2,
    const float*  LET_MeV_g_cm2,
    const float*  dfluencedz_cm,
    const float*  E_MeV,
    float*  dose_Gy,
    float*  dose_noNuc_Gy)
{
...
void AT_BortfeldTransportProton( const float*  E_initial_MeV,
    const float*  sE_initial_MeV,
    const float*  fluence_initial_cm2,
    const char*  plateau_dose_material_name,
    const long*  n_shielding_slabs,
    const float*  shielding_thickness_m,
    const char**  shielding_material_name,
    const long*  n_detector_slabs,
    const float*  detector_thickness_m,
    const char*  detector_material_name,
    /* return values */
    float*  plateau_dose_Gy,
    float*  plateau_dose_noNuc_Gy,
    float*  detector_z_cm,
    ...
    float*  geom_factor)
{
  ...
  float fluence_initial_cm2_copy = *fluence_initial_cm2;
  if(*fluence_initial_cm2 < 0){
    fluence_initial_cm2_copy = -1.0f * (*fluence_initial_cm2) / (LET_MeV_g_cm2 * MeV_to_J * g_cm3_to_kg_m3);
  }
  float dfluencedz = (fluence_initial_cm2_copy) * m_cm[0] / (1 + m_cm[0] * R0_cm);

  AT_Dose_Gy(  &density_g_cm3[0],
      &fluence_initial_cm2_copy,
      &LET_MeV_g_cm2,
      &dfluencedz,
      E_initial_MeV,
      // return
  ...
  float  fluence_cm2  =  fluence_initial_cm2_copy;
