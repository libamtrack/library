    ...
    float         f1_parameters[])
{
  ...
  float single_impact_fluence_cm2_float = 0.0f;
  long  n_tmp    = 1;
  ...
  AT_single_impact_fluence_cm2( &n_tmp,
      ...
      &material_no,
      &er_model,
      &single_impact_fluence_cm2_float);
  single_impact_fluence_cm2 = (double)single_impact_fluence_cm2_float;
  ...
  if( rdd_model == RDD_KatzPoint){
    ...
    // 2. calculate minimum dose d_min_Gy (f1_parameters[3])
    d_min_Gy              =  (double)rdd_parameter[1];                                                 // d_min_Gy given as model parameter
  ...
  if( rdd_model == RDD_Site){
    ...
    if( (er_model == ER_Waligorski) || (er_model == ER_Edmund) ){ // calculate dEdx_MeV_cm2_g from "new" Katz RDD
      ...
      d_max_Gy            =  AT_RDD_Katz_PowerLawER_DSite_Gy(0.0, r_min_m, max_electron_range_m, density_kg_m3, alpha, LET_J_m, dEdx_J_m, Katz_point_coeff_Gy);
      ...
      d_max_Gy            =  AT_RDD_Katz_LinearER_DSite_Gy(0.0, r_min_m, max_electron_range_m, density_kg_m3, LET_J_m, dEdx_J_m, Katz_point_coeff_Gy);
    } else {
      d_max_Gy            =  0.0;
    }
    ...
    // 6. calculate dEdx_MeV_cm2_g (f1_parameters[8])
    dEdx_MeV_cm2_g        =  dEdx_J_m / 100.0 / density_g_cm3 / MeV_to_J;
  ...
  if( rdd_model == RDD_Edmund){
    ...
    if( (er_model == ER_Waligorski) || (er_model == ER_Edmund) ){ // This model will work only with ER_Waligorski and ER_Edmund
      ...
      dEdx_J_m            =  AT_RDD_Katz_PowerLawER_dEdx_directVersion_J_m((float)alpha, (float)r_min_m, (float)max_electron_range_m, (float)Katz_dEdx_coeff_J_m);
      ...
      if(tmp > 0){
        ...
        tmp *=  MeV_to_J;      // J / cm
        tmp *=  100.0;        // J / m
        double core_kg_m   =  M_PI * gsl_pow_2(rdd_parameter[0]) * density_kg_m3;  // kg / m
  ...
  if( rdd_model == RDD_Geiss){
    ...
    double a0_m           =  0.0;
    a0_m                  =  (double)rdd_parameter[0];                                            // "r_min_m" = a0
  ...
  if( rdd_model == RDD_KatzExtTarget){ // TODO To be removed
    ...
    // 3. calculate maximum dose d_max_Gy (f1_parameters[4])
    const long   n_tmp     =  1;
    const double a0_m      =  (double)rdd_parameter[1];
  ...
  if( rdd_model == RDD_CucinottaExtTarget){ // TODO To be removed
    ...
    float d_max_Gy_float;
    AT_RDD_ExtendedTarget_Gy(n_tmp, &r_min_m_float, (float)a0_m, (float)E_MeV_u, particle_no, material_no, RDD_Cucinotta, rdd_basic_parameter, er_model, er_parameter, &d_max_Gy_float);
    d_max_Gy = (double)d_max_Gy_float;
  ...
  f1_parameters[8]  =  (float)dEdx_MeV_cm2_g;
